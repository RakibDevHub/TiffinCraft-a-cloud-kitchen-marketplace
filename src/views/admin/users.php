                            ...
                            <tr class="hover:bg-gray-50" data-role="<?= strtolower($user['role']) ?>">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    ...
                                    <div class="text-sm text-gray-500">
                                        <?= !empty($user['phone_number']) ? htmlspecialchars($user['phone_number']) : 'No phone' ?>
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <?php
                                ...
                                <td class="px-6 py-4">
                                    <div class="text-sm text-gray-900">
                                        <?= !empty($user['address']) ? htmlspecialchars($user['address']) : 'No address' ?>
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    <?php
                                    try {
                                        // Handle Oracle TIMESTAMP format
                                        $date = DateTime::createFromFormat('d-M-y H.i.s.u A', $user['created_at']);
                                        echo $date ? $date->format('M j, Y \a\t g:i A') : 'Invalid date';
                                    } catch (Exception $e) {
                                        echo 'Date error';
                                    }
                                    ?>
...
<script>
    ...
    function filterUsers() {
        ...
        document.querySelectorAll('#userTableBody tr').forEach(row => {
            ...
            // Show if matches both search term and role filter
    ...
    function openEditModal(user) {
        ...
        // You would populate a modal form with the user data here
    }
</script>
